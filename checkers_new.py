...
class Graphics:
	def __init__(self):
		...
		self.background = pygame.image.load('board2.jpg')
		

		self.square_size = self.window_size / 8
		#print self.square_size
		#self.square_size = 175
	...
	def update_display(self, board, legal_moves, selected_piece):
		...
		"""
		self.screen.blit(self.background, (-2,-4))
...
class Board:
	...
	def king(self, (x,y)):
		...
		if self.location((x,y)).occupant != None:
			if (self.location((x,y)).occupant.color == BLACK and y == 0) or (self.location((x,y)).occupant.color == RED and y == 7):
